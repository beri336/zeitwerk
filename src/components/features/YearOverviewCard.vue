  ...
  <main class="main">
    ...
    <section class="year-grid" aria-label="Year calendar overview">
      <article
        ...
        class="year-month"
        :class="[
          monthCardClass(month),
          { 'year-month--current': isCurrentMonthCard(month.index) },
        ]"
      >
        <header class="year-month-header">
          ...
          <div class="year-month-meta">
            <span>{{
              $t("common.entries_count", { count: month.stats.count })
            }}</span>
            <span :class="month.stats.diff >= 0 ? 'stat-ok' : 'stat-err'">
              {{ month.stats.diff >= 0 ? "+" : ""
              }}{{ formatHours(month.stats.diff) }}
            </span>
          </div>
        </header>
...
function getMonthStats(month) {
  ...
  const actual = monthEntries.reduce(
    (sum, entry) => sum + store.effectiveActualHours(entry),
    0,
  );
...
const yearMonths = computed(() =>
  Array.from({ length: 12 }, (_, monthIndex) => {
    const name = new Date(store.currYear, monthIndex, 1).toLocaleDateString(
      currentLocale.value,
      { month: "long" },
    );
...
function monthCardClass(month) {
  if (month.stats.diff > 0.1) return "year-month--positive";

  if (month.stats.diff < -0.1) return "year-month--negative";
...
const yearGross = computed(() =>
  yearEntries.value.reduce(
    (sum, entry) => sum + store.grossEarnedForEntry(entry),
    0,
  ),
);

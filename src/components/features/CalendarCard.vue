    ...
    <div class="cal-wrapper">
      ...
      <Transition name="slide-up">
        <div v-if="selectedDate && activeView !== 'agenda'" class="day-detail">
          ...
          <div class="day-detail-empty">{{ $t("calendar.no_events") }}</div>
        </div>
      </Transition>
    </div>
          ...
          <div class="modal">
            <header class="modal-header">
              <h2 class="modal-title">
                {{
                  modal.editId
                    ? $t("calendar.modal.edit_appointment")
                    : $t("calendar.modal.new_appointment")
                }}
              </h2>
            ...
            <div class="modal-body">
              ...
              <div class="field">
                ...
                <select v-model="form.reminder" class="form-input form-select">
                  ...
                  <option value="0">
                    {{ $t("calendar.modal.reminder_0") }}
                  </option>
                  <option value="5">
                    {{ $t("calendar.modal.reminder_5") }}
                  </option>
                  <option value="15">
                    {{ $t("calendar.modal.reminder_15") }}
                  </option>
                  <option value="30">
                    {{ $t("calendar.modal.reminder_30") }}
                  </option>
                  <option value="60">
                    {{ $t("calendar.modal.reminder_60") }}
                  </option>
                  <option value="1440">
                    {{ $t("calendar.modal.reminder_1440") }}
                  </option>
                </select>
              </div>
            </div>
...
const weekdays = computed(() => {
  const base = new Date(2023, 0, 2); // ein Montag als Ankerpunkt
...
function eventsForDate(dateStr) {
  return events.value
    ...
    .sort(
      (a, b) => (a.allDay ? -1 : 1) || a.startTime.localeCompare(b.startTime),
    );
}
...
const currentLocale = computed(() => currentLocale.value);
...
const agendaGroups = computed(() => {
  ...
  events.value
    ...
    .forEach((entry) => {
      const label = new Date(entry.date + "T00:00").toLocaleDateString(lang, {
        weekday: "long",
        day: "2-digit",
        month: "long",
      });
  ...
  return [...map.entries()].map(([label, evts]) => ({ label, events: evts }));
});
...
const selectedDateLabel = computed(() => {
  ...
  return new Date(selectedDate.value + "T00:00").toLocaleDateString(
    currentLocale.value,
    {
      weekday: "long",
      day: "2-digit",

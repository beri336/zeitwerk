...
<template>
    <div class="habit-wrapper">
        ...
        <div class="week-strip">
            <div v-for="day in weekStrip" :key="day.dateStr" class="week-day"
                :class="{ 'week-day--today': day.isToday, 'week-day--selected': selectedDate === day.dateStr }"
        ...
        <div class="date-label">
            {{ selectedDateLabel }}
            <span v-if="selectedDate === todayStr" class="today-chip">Today</span>
        </div>
        ...
        <TransitionGroup name="habit-list" tag="div" class="habit-list">
            <div v-for="habit in visibleHabits" :key="habit.id" class="habit-item"
                ...
                <!-- Check Button -->
                <button class="habit-check" :style="{ borderColor: habit.color }"
                    :class="{ 'habit-check--done': isDone(habit.id, selectedDate) }"
                    @click="toggleHabit(habit.id, selectedDate)">
                    <svg v-if="isDone(habit.id, selectedDate)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                    ...
                    <div class="habit-meta">
                        ...
                        <div class="mini-calendar">
                            <div v-for="d in last7(habit.id)" :key="d.dateStr" class="mini-dot"
                                ...
                                :style="d.done ? { background: habit.color } : {}" :title="d.dateStr">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="habit-actions">
                    <button class="icon-btn" @click="openModal(habit.id)" title="Bearbeiten">
                    ...
                    <button class="icon-btn icon-btn--danger" @click="removeHabit(habit.id)" title="Löschen">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="13"
                            height="13">
                            <path fill-rule="evenodd"
        ...
        <div v-if="habits.length > 0" class="stats-bar">
            ...
            <div class="stat-item">
                ...
                <span class="stat-label">This week rate</span>
            </div>
        </div>

        <!-- View Toggle -->
        <div v-if="habits.length > 0" class="view-toggle">
            <button v-for="v in historyViews" :key="v.key" class="freq-btn" :class="{ active: historyView === v.key }"
        ...
        <!-- Month View -->
        <div v-if="habits.length > 0 && historyView === 'month'" class="history-card">
            <div class="history-header">
                <button class="icon-btn" @click="prevMonth">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="15"
                        height="15">
                        <path fill-rule="evenodd"
            ...
            <div class="month-grid">
                <div v-for="cell in monthCells" :key="cell.key" class="month-cell" :class="{
                    ...
                    <div v-if="cell.currentMonth" class="month-cell-bar">
                        <div class="month-cell-bar-fill" :style="{
        ...
        <!-- Year View -->
        <div v-if="habits.length > 0 && historyView === 'year'" class="history-card">
            <div class="history-header">
                ...
                <button class="icon-btn" @click="viewYear++" :disabled="viewYear >= today.getFullYear()">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="15"
                        height="15">
                        <path fill-rule="evenodd"
                            d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z"
            ...
            <div class="year-habit-tabs">
                <button v-for="habit in habits" :key="habit.id" class="year-habit-tab"
                    :class="{ active: yearHabitId === habit.id }"
                    :style="yearHabitId === habit.id ? { background: habit.color + '33', borderColor: habit.color, color: habit.color } : {}"
            ...
            <div class="year-heatmap-scroll">
                <div class="year-heatmap">
                    <div v-for="month in yearMonths" :key="month.label" class="year-month-col">
                        <div class="year-month-label">{{ month.label }}</div>
                        <div class="year-month-days">
                            <!-- spacer for monday alignment -->
                            <div v-for="n in month.startOffset" :key="'offset-' + n" class="year-day year-day--empty">
                            </div>
                            <div v-for="day in month.days" :key="day.dateStr" class="year-day" :class="{
                                'year-day--today': day.dateStr === todayStr,
                                'year-day--future': day.isFuture,
                            }" :style="isDone(yearHabitId, day.dateStr) ? { background: selectedYearHabit?.color, opacity: 1 } : {}"
                                :title="day.dateStr">
                            </div>
                        </div>
                    </div>
            ...
            <div class="year-stats" v-if="selectedYearHabit">
                <div class="year-stat">
                    ...
                    <span class="year-stat-lbl">Days completed</span>
                </div>
                <div class="stat-divider"></div>
                <div class="year-stat">
                    <span class="year-stat-val">{{ yearRate }}%</span>
                    <span class="year-stat-lbl">Completion rate</span>
                </div>
                <div class="stat-divider"></div>
                <div class="year-stat">
                    <span class="year-stat-val">{{ yearBestStreak }}</span>
                    <span class="year-stat-lbl">Best streak</span>
                </div>
            </div>
        </div>
                ...
                <div v-if="modal.open" class="modal-backdrop" @mousedown.self="closeModal">
                    <div class="modal">
                        <header class="modal-header">
                            <h2 class="modal-title">{{ modal.editId ? 'Edit Habit' : 'New Habit' }}</h2>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="field-row">
                                <div class="field field--icon">
                                    ...
                                    <input v-model="form.icon" class="form-input icon-input" type="text" maxlength="2"
                                        placeholder="🏃" />
                                </div>
                                <div class="field">
                            ...
                            <div class="field">
                                ...
                                <input v-model="form.goal" class="form-input" type="text"
                                    placeholder="e.g. 30 min, 2km, 8 glasses" />
                            </div>
                            ...
                            <div class="field">
                                ...
                                <div class="color-selector">
                                    <button v-for="color in colorOptions" :key="color" class="color-btn"
                                        ...
                                        <svg v-if="form.color === color" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 20 20" fill="white" width="11" height="11">
                                            <path fill-rule="evenodd"
                                                d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z"
                                                clip-rule="evenodd" />
                                        </svg>
                            ...
                            <div class="field">
                                ...
                                <div class="freq-selector">
                                    <button v-for="freq in frequencies" :key="freq.key" class="freq-btn"
                        ...
                        <footer class="modal-footer">
                            ...
                            <button class="btn btn-primary" :disabled="!form.name.trim()" @click="saveModal">
...
const totalCompletionsThisWeek = computed(() => {
    ...
    return habits.value.reduce((sum, h) =>
        sum + dates.filter(d => isDone(h.id, d)).length, 0
    )
})
...
const canGoNextMonth = computed(() => {
    if (viewMonthYear.value < today.getFullYear())
        return true
...
const monthLabel = computed(() =>
    new Date(viewMonthYear.value, viewMonth.value, 1)
...
const monthCells = computed(() => {
    ...
    for (let i = startDow - 1; i >= 0; i--) {
        ...
        cells.push({ key: 'pre-' + dateStr, day: d.getDate(), dateStr, currentMonth: false })
    }
    ...
    for (let i = 1; i <= remaining; i++) {
        ...
        const dateStr = toDateStr(d)
        cells.push({ key: 'post-' + dateStr, day: d.getDate(), dateStr, currentMonth: false })
    }
    return cells
})
...
function cellCompletionColor(dateStr) {
    ...
    if (rate === 0)
        return 'var(--color-border, #2d3148)'
...
watch(habits, (val) => {
    if (!yearHabitId.value && val.length)
        yearHabitId.value = val[0].id
}, { immediate: true })
...
const yearMonths = computed(() => {
    ...
    return Array.from({ length: 12 }, (_, m) => {
        const label = new Date(year, m, 1).toLocaleDateString('de-DE', { month: 'short' })

    ...
    <main class="main">
        <div class="todo-wrapper">
            ...
            <header class="todo-header">
                <h1 class="todo-title">To-Do's Overview</h1>
            ...
            <div v-if="allTags.length" class="tag-filter-row">
                <button class="tag tag--filter" :class="{ active: !activeTag }" @click="activeTag = null">Alle</button>
            ...
            <TransitionGroup name="todo-list" tag="ul" class="todo-list">
                <li v-for="todo in filteredTodos" :key="todo.id" class="todo-item"
                    :class="{ done: todo.done, urgent: todo.urgent, [`priority-${todo.priority}`]: true }">

                    <!-- Left: Checkbox -->
                    <button class="todo-check" @click="toggleTodo(todo.id)">
                        <svg v-if="todo.done" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                            width="14" height="14">
                    ...
                    <div class="todo-content">
                        ...
                        <!-- Meta Row -->
                        <div class="todo-meta">
                            <span v-if="todo.dueDate || todo.dueTime" class="meta-item"
                        ...
                        <div v-if="todo.subtasks.length" class="subtasks">
                            ...
                            <span class="subtask-count">
                                {{todo.subtasks.filter(sub => sub.done).length}}/{{ todo.subtasks.length }}
                                Unteraufgaben
                            </span>
                            <ul class="subtask-list" v-show="todo.expanded">
                                <li v-for="sub in todo.subtasks" :key="sub.id" class="subtask-item"
                                    :class="{ done: sub.done }">
                                    <button class="todo-check todo-check--sm" @click="toggleSubtask(todo.id, sub.id)">
                                        <svg v-if="sub.done" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                    ...
                                    <div class="subtask-meta">
                                        <span v-if="sub.dueDate || sub.dueTime" class="meta-item"
                                    ...
                                    <div class="todo-actions subtask-actions">
                                        <button class="icon-btn" @click="openModal(todo.id, sub.id)" title="Bearbeiten">
                                        ...
                                        <button class="icon-btn icon-btn--danger"
                                            @click="removeSubtask(todo.id, sub.id)" title="Löschen">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                                fill="currentColor" width="12" height="12">
                                                <path fill-rule="evenodd"
                    ...
                    <div class="todo-actions">
                        <button class="icon-btn" title="Add Subtask" @click="openModal(todo.id, null, true)">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14"
                                height="14">
                                <path
                                    d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
                            </svg>
                        </button>
                        <button class="icon-btn" @click="openModal(todo.id)" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14"
                                height="14">
                                <path
                                    d="M2.695 14.763l-1.262 3.154a.5.5 0 00.65.65l3.155-1.262a4 4 0 001.343-.885L17.5 5.5a2.121 2.121 0 00-3-3L3.58 13.42a4 4 0 00-.885 1.343z" />
                        ...
                        <button class="icon-btn icon-btn--danger" @click="removeTodo(todo.id)" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14"
                                height="14">
                                <path fill-rule="evenodd"
                                    d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.841 10.518A2.75 2.75 0 007.596 19h4.807a2.75 2.75 0 002.742-2.53l.841-10.52.149.023a.75.75 0 00.23-1.482A41.03 41.03 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z"
            ...
            <footer v-if="todos.length > 0" class="todo-footer">
                <span>{{ remaining }} of {{ todos.length }} pending</span>
                    ...
                    <div class="modal">
                        <header class="modal-header">
                            <h2 class="modal-title">
                                {{ modal.isSubtask ? 'Subtask' : 'Task' }}
                                {{ modal.editId ? 'Edit' : 'Create' }}
                            </h2>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="field">
                                ...
                                <input v-model="form.text" class="todo-input" type="text"
                            ...
                            <div class="field">
                                ...
                                <div class="tag-input-wrapper">
                                    ...
                                    <input v-model="tagInput" class="tag-input" type="text"
                                        placeholder="Enter the tag + press Enter" @keydown.enter.prevent="addTag"
                                ...
                                <div class="field-row">
                                    <div class="field">
                                        ...
                                        <div class="priority-selector">
                                            <button v-for="priority in priorities" :key="priority.key"
                                                class="priority-btn"
                                                :class="[`priority-btn--${priority.key}`, { active: form.priority === priority.key }]"
...
let nextId = todos.value.length
    ? Math.max(...todos.value.flatMap(t => [t.id, ...t.subtasks.map(s => s.id)])) + 1
    : 1
...
function isOverdue(item) {
    ...
    const due = new Date(`${item.dueDate}${item.dueTime ? 'T' + item.dueTime : 'T23:59'}`)
...
function subtaskProgress(todo) {
    ...
    return (todo.subtasks.filter(s => s.done).length / todo.subtasks.length) * 100
}
...
const emptyMessage = computed(() => {
    if (activeFilter.value === 'done')
        return 'Nothing has been done yet. Get to work! 💪'

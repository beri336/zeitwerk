...
<template>
  ...
  <div class="table-wrap">
    ...
    <div v-if="store.entriesForMonth.length === 0" class="empty-state">
      ...
      <h3>
        {{ $t("monthTable.empty_title", { month: store.currMonthLabel }) }}
      </h3>
      <p>{{ $t("monthTable.empty_sub") }}</p>
    </div>
    ...
    <div v-else class="table-scroll">
      <table class="data-table">
        <thead>
          <tr>
            <th>{{ $t("monthTable.col_date") }}</th>
            <th>{{ $t("monthTable.col_time_entries") }}</th>
            <th>{{ $t("monthTable.col_pauses") }}</th>
            <th class="num">{{ $t("monthTable.col_actual_day") }}</th>
            <th class="num">{{ $t("monthTable.col_planned_day") }}</th>
            <th>{{ $t("monthTable.col_notes") }}</th>
            <th class="actions-header">{{ $t("monthTable.col_actions") }}</th>
          </tr>
        </thead>
        <tbody>
          <template
          ...
          >
            ...
            >
              ...
              <td class="num">
                {{
                  formatHours(entry.plannedHours || store.settings.hoursPerDay)
                }}
              </td>
...
function actualVariant(entry) {
  ...
  if (Actual >= planned - 0.1) return "cell-ok";
  if (Actual > 0) return "cell-warn";
